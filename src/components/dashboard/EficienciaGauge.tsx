...
export function EficienciaGauge({ dados, metas, iso }: EficienciaGaugeProps) {
  ...
  // Recalcula conforme os lançamentos e prazos da data selecionada
  const valor = useMemo(
    () => calcularEficiencia(dados, metas, iso),
    [dados, metas, iso]
  );
  const nivel = nivelEficiencia(valor);
  const config = NIVEIS_EFICIENCIA[nivel];
  const metasAtivas = METRICAS_META.some(({ tipo }) => !!metas[tipo].horario);
  ...
  return (
    <div className="flex flex-col rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="flex flex-1 flex-col items-center justify-center">
        ...
        {metasAtivas ? (
          <p className="mt-1.5 text-center text-[10.5px] font-medium text-slate-400 dark:text-slate-500">
            Lançamentos de {formatarData(iso)}
          </p>
        ) : (
