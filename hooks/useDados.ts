...
import { METAS_PADRAO } from "@/lib/constantes";
...
export function useDados() {
  ...
  const [metas, setMetas] = useState<MetasOperacionais>(METAS_PADRAO);
  ...
  useEffect(() => {
    // Carrega metas e marca como carregado no mount
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setMetas(carregarMetas());
    setCarregado(true);
  }, []);
  ...
  useEffect(() => {
    if (origem !== "supabase" || !supabaseConfig) {
      if (origem === "local") {
        // Carrega os dados locais ao entrar no modo local
        // eslint-disable-next-line react-hooks/set-state-in-effect
  ...
  const atualizarMetas = useCallback((novas: MetasOperacionais) => {
    setMetas(novas);
    salvarMetas(novas);
  }, []);
